...
const io = new Server(httpServer, {
  cors: {
    origin: config.AGENT_CLIENT_URL,
    methods: ['GET', 'POST'],
    credentials: true,
  },
});
...
httpServer.listen(Number(config.ANALYTICS_PORT), () => {
  console.log(
    `🚀 Analytics server is running on port ${config.ANALYTICS_PORT} in ${config.NODE_ENV}`
  );
});
